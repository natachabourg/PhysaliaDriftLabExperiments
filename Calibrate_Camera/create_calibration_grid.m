...
x% From the image with the lines, input lines, find intersections, then put
% them in a nice matrix with the right order
...
% Result is 2 matrices, one zith x ccord, one with y coord
figure()
scatter(Final_gridpoints_order_x,Final_gridpoints_order_y)

save('data_coord_grid_good_order')
save('data_coord_grid_matrix_camera.mat','Final_gridpoints_order_x','Final_gridpoints_order_y')% s_len = length(xs)
nb_lines1 = input('How many points in lines done first ( vertical or horizontal)') 
